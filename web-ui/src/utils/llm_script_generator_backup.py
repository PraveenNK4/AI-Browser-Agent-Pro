...
def _build_system_prompt(vault_prefix: str = None) -> str:
    ...
    vault_prefix    = vault_prefix or VAULT_CREDENTIAL_PREFIX
...
3.  **Login hook** — Call `await maybe_login(page, vault_prefix="{vault_prefix}")` once, right after the FIRST `page.goto(...)`.
    Then, check if the landing page requires an SSO activation (e.g., a "Sign in" button to activate the session).
    If found, click it: `await resilient_click(page, "text=\'Sign in\'")` and wait for redirection.
    Do NOT repeat login after subsequent navigations — the session persists.
...
6.  **Selectors** — Use ONLY selectors from the NAVIGATION PLAN or SELECTOR REFERENCE TABLE.
    ...
    **SPA Navigation**: For SPAs (like IAWA), if `page.goto` redirects back to a default view,
    use `await page.evaluate("window.location.hash = \'#/target/path\'")` to transition.
...
10. **SVG Extraction** — To extract center-text from charts:
    ```python
    val = await page.evaluate(r\'\'\'() => {{
        const findValue = (root) => {{
            const texts = Array.from(root.querySelectorAll(\'text\')).map(t => t.textContent.trim());
            const num = texts.find(t => /^\\d+\\.\\d+$/.test(t));
            const unit = texts.find(t => /^(GB|TB)$/i.test(t));
            return (num && unit) ? `${{num}} ${{unit}}` : (num || null);
        }};
        const chart = document.querySelector(\'#chart-id\') || document.querySelector(\'svg\');
        return findValue(chart) || "Not Found";
    }}\'\'\')
    ```
...
11. **Comprehensive Reporting** — ALWAYS `print()` extracted data to the terminal.
    ...
    CRITICAL: ALWAYS use `await generate_report(full_markdown_report, steps=steps)`. 
    NEVER redefine `generate_report` or write your own file-writing logic.
...
async def run():
    async with async_playwright() as p:
        ...
        await maybe_login(page, vault_prefix="{vault_prefix}") # Use prefix if detected
        await page.wait_for_load_state("networkidle")
        
        # Optional: Activate SSO if landing page appears
        if await page.get_by_text("Sign in").is_visible():
            await resilient_click(page, "text=\'Sign in\'", desc="SSO Activation")
            await page.wait_for_load_state("networkidle")
        ...
        await page.wait_for_load_state("networkidle") # MANDATORY
        ...
        # ── Step A.1: SVG Chart Center Extraction (Pattern 23) ──────────────
        # Capture name and split value from SVG chart
        chart_data = await page.evaluate(r\'\'\'() => {{
            const chart = document.querySelector(\'#storage-chart\');
            if (!chart) return "Not Found";
            const texts = Array.from(chart.querySelectorAll(\'text\')).map(t => t.textContent.trim());
            // Combine numeric part and unit if split
            const num = texts.find(t => /^\\d+\\.\\d+$/.test(t));
            const unit = texts.find(t => /^(GB|TB)$/i.test(t));
            return (num && unit) ? `${{num}} ${{unit}}` : (num || "Not Found");
        }}\'\'\')
        await capture_step("Extract Chart", f"Found value: {{chart_data}}")

        # ── Step 3: Page B — SPA Navigation & Table ───────────────────────
        # Using hash navigation if direct goto is unstable
        await page.evaluate("window.location.hash = \'#/dashboard/storage-dashboard\'")
        await page.wait_for_load_state("networkidle")
        await asyncio.sleep(5) # Allow charts to render
...
# NOTE: SYSTEM_PROMPT is now built per generate_script() call so the vault_prefix
# can be injected per task. The module-level constant is kept only for callers that
# don't go through generate_script() (e.g. unit tests).
...
def _postprocess_generated_code(code: str, cleaned_history_json: str, tbl_registry: dict | None = None) -> str:
    ...
    if len(entrypoint_positions) > 1:
        ...
        # Matches any form: maybe_login(page) or maybe_login(page, vault_prefix="...")
        first_login_seen = False
        deduped = []
        for line in merged_body.splitlines(keepends=True):
            if re.search(r'await\s+maybe_login\s*\(', line):
                if first_login_seen:
                    logger.info("[POST] Removed duplicate maybe_login call")
                    continue
    ...
    # ── 10: Normalise hardcoded vault key names → per-task effective prefix ─────
    # LLM sometimes emits get_secret("OTCS_USERNAME") with the wrong literal prefix,
    # or get_secret(f"{VAULT_CREDENTIAL_PREFIX}_USERNAME") referencing the global config.
    # We want the generated script to use the specific vault prefix for THIS task
    # (e.g. "GI07"), not the global default, so the script is self-contained.
    _vault_key_pat = re.compile(
        r'get_secret\(\s*["\']([A-Z][A-Z0-9]*)_(USERNAME|PASSWORD|USER|PWD)["\']'
        r'\s*\)'
    )

    # Derive the effective prefix used for normalization:
    # selector/required come from _extract_table_hints which doesn't know about vault,
    # so we look for @vault.XXX in the cleaned history JSON itself.
    _pp_vault_prefix = None
    try:
        _pp_vm = re.findall(r'@vault\.([a-zA-Z0-9_]+)', cleaned_history_json)
        if _pp_vm:
            _pp_vault_prefix = _pp_vm[0].upper()
    except Exception:
        pass
    # Fall back to global config only if history has no @vault reference
    _pp_effective = _pp_vault_prefix or VAULT_CREDENTIAL_PREFIX

    def _normalize_vault_key(m):
        suffix = m.group(2)  # USERNAME / PASSWORD / USER / PWD
        return f'get_secret("{_pp_effective}_{suffix}")'

    if _vault_key_pat.search(code):
        def _maybe_replace(m):
            prefix_in_code = m.group(1)
            suffix = m.group(2)
            full_key = f"{prefix_in_code}_{suffix}"
            # Already correct: skip
            if prefix_in_code == _pp_effective:
                return m.group(0)
            logger.info(f"[POST] Normalised vault key: {full_key!r} → {_pp_effective}_{suffix}")
            return _normalize_vault_key(m)
        code = _vault_key_pat.sub(_maybe_replace, code)

    # Also rewrite the f-string form: get_secret(f"{VAULT_CREDENTIAL_PREFIX}_USERNAME")
    # → get_secret("GI07_USERNAME") so the script works standalone without config import.
    if _pp_vault_prefix:
        code = re.sub(
            r'get_secret\(f"\{VAULT_CREDENTIAL_PREFIX\}_(USERNAME|PASSWORD|USER|PWD)"\)',
            lambda m: f'get_secret("{_pp_effective}_{m.group(1)}")',
            code,
        )

    # Fix maybe_login calls that are missing vault_prefix or have the wrong one
    # Rewrite: maybe_login(page) → maybe_login(page, vault_prefix="GI07")
    # Rewrite: maybe_login(page, vault_prefix="OTCS") → maybe_login(page, vault_prefix="GI07")
    if _pp_vault_prefix:
        # Bare call: maybe_login(page)
        code = re.sub(
            r'maybe_login\(\s*page\s*\)',
            f'maybe_login(page, vault_prefix="{_pp_effective}")',
            code,
        )
        # Call with wrong prefix
        code = re.sub(
            r'maybe_login\(\s*page\s*,\s*vault_prefix\s*=\s*["\'][^"\']+["\']\s*\)',
            f'maybe_login(page, vault_prefix="{_pp_effective}")',
            code,
        )
    ...
    # ── 11b: Global maybe_login deduplication ─────────────────────────────────
    # Catches duplicates emitted by the LLM itself (not just from script merging).
    # Only the FIRST maybe_login call is kept — the session persists across gotos.
    _ml_seen = False
    _dedup_lines = []
    for _dl in code.splitlines(keepends=True):
        if re.search(r'await\s+maybe_login\s*\(', _dl):
            if _ml_seen:
                logger.info(f"[POST] Removed duplicate maybe_login call: {_dl.strip()}")
                continue
            _ml_seen = True
        _dedup_lines.append(_dl)
    code = "".join(_dedup_lines)
    ...
    if not re.search(r'maybe_login\s*\(', code):
    ...
    if issues:
        ...
        # Prepend warnings but ALWAYS return the code — a partially-correct script
        # with visible warnings is far more useful than no script at all.
        # (Previously this raised RuntimeError when selector was None, causing the
        # UI to show "# Script generation not available or disabled" instead.)
        code = warning + code
...
def _try_template_fast_path(
    ...
    # Login block
    _vp = (vault_prefix or VAULT_CREDENTIAL_PREFIX).upper()
    L.append("        # ── Step 1: Login")
    L.append("        await page.goto(" + repr(login_url) + ", timeout=TIMEOUT_TABLE_WAIT_MS)")
    L.append(f'        await maybe_login(page, vault_prefix="{_vp}")')
...
def generate_script(history_path: str, output_path: str = None, model_name: str = None, provider: str = None, objective: str = None, mandatory_history_path: str = None, vault_prefix: str = None):
    """
    ...
    Args:
        ...
        vault_prefix: Vault key prefix for credentials (e.g. "gi07", "OTCS").
                      Overrides VAULT_CREDENTIAL_PREFIX from config for this script only.
                      Derived automatically from @vault.XXX references in the task when
                      called from browser_use_agent_tab.
    """
    ...
    try:
        ...
        # ── Auto-detect vault prefix from @vault.XXX in history objective ────
        # Covers: CLI calls without --vault-prefix, webui script regeneration,
        # and any direct generate_script() call that omits vault_prefix.
        if not vault_prefix:
            _obj_text = (objective or history_data.get('task', '') or '')
            _vm = re.findall(r'@vault\.([a-zA-Z0-9_]+)', _obj_text)
            if _vm:
                vault_prefix = _vm[0].upper()
                logger.info(f"[VAULT] Auto-detected vault prefix '{vault_prefix}' from history objective")
        ...
        # Build system prompt with the per-task vault prefix so the LLM uses the
        # correct vault key in get_secret() and maybe_login() calls.
        _effective_prefix = (vault_prefix or VAULT_CREDENTIAL_PREFIX).upper()
        _system_prompt = _build_system_prompt(vault_prefix=_effective_prefix)

        # Prepend a concise vault-prefix note to the user prompt so the LLM never
        # guesses or hardcodes credentials.
        _vault_note = (
            f"## VAULT PREFIX FOR THIS SCRIPT\n"
            f"Credentials are stored under vault key **\"{_effective_prefix}\"**.\n"
            f"- Username: `await get_secret(\"{_effective_prefix}_USERNAME\")`\n"
            f"- Password:  `await get_secret(\"{_effective_prefix}_PASSWORD\")`\n"
            f"- Login hook: `await maybe_login(page, vault_prefix=\"{_effective_prefix}\")`\n"
            f"NEVER use any other vault prefix. NEVER hardcode credentials.\n\n"
        )
        user_prompt = _vault_note + user_prompt
        logger.info(f"[VAULT] Using vault prefix '{_effective_prefix}' for script generation")
        ...
        # Returns code with warning comments prepended if issues were found — never raises
        _tbl_registry_safe = locals().get('_tbl_registry', None)
        try:
            code = _postprocess_generated_code(code, cleaned_history, tbl_registry=_tbl_registry_safe)
        except Exception as qg_err:
            logger.error(f"[QUALITY GATE] Post-processing failed unexpectedly: {qg_err}")
            # Still save whatever the LLM generated with an error header
...
if __name__ == "__main__":
    from src.utils.config import SCRIPT_GEN_MODEL, SCRIPT_GEN_PROVIDER, VAULT_CREDENTIAL_PREFIX
    ...
    parser.add_argument("--vault-prefix", default=None,
                        help=f"Vault credential prefix (e.g. 'gi07', 'OTCS'). "
                             f"Auto-detected from @vault.XXX in history objective if omitted. "
                             f"Falls back to VAULT_CREDENTIAL_PREFIX env (currently '{VAULT_CREDENTIAL_PREFIX}').")

    args = parser.parse_args()

    # Auto-detect vault prefix from history objective if not supplied
    _cli_vault_prefix = args.vault_prefix
    if not _cli_vault_prefix:
        try:
            with open(args.history_file, 'r', encoding='utf-8') as _f:
                _hist = json.load(_f)
            _obj = _hist.get('task', '') or _hist.get('objective', '')
            _vm = re.findall(r'@vault\.([a-zA-Z0-9_]+)', _obj)
            if _vm:
                _cli_vault_prefix = _vm[0].upper()
                print(f"[auto-detect] vault prefix from history objective: '{_cli_vault_prefix}'")
        except Exception:
            pass

    generate_script(args.history_file, args.output, args.model, args.provider,
                    vault_prefix=_cli_vault_prefix)