...
class CustomController(Controller):
    ...
    def _register_custom_actions(self):
        ...
        @self.registry.action(
            'Click on an element that contains the specified text. Use this when element indices are unreliable.',
        )
        async def click_element_by_text(text: str, browser: BrowserContext):
            try:
                # Use Playwright's get_by_text to find and click the element
                locator = browser.page.get_by_text(text, exact=False)
                await locator.click()
                msg = f'Successfully clicked element containing text: {text}'
                logger.info(msg)
                return ActionResult(extracted_content=msg, include_in_memory=True)
            except AttributeError as e:
                # If browser doesn't have page, try accessing _page
                if hasattr(browser, '_page'):
                    locator = browser._page.get_by_text(text, exact=False)
                    await locator.click()
                    msg = f'Successfully clicked element containing text: {text}'
                    logger.info(msg)
                    return ActionResult(extracted_content=msg, include_in_memory=True)
                ...
                msg = f'Failed to click element containing text "{text}": {str(e)}'
                logger.info(msg)
                return ActionResult(error=msg)

        @self.registry.action(
            'Zoom out the page to a specified percentage (e.g., 80 for 80%) to make more content visible without scrolling.',
        )
        async def zoom_page(percentage: int, browser: BrowserContext):
            try:
                await browser.page.evaluate(f"document.body.style.zoom = '{percentage}%'")
                msg = f'Successfully zoomed page to {percentage}%'
                logger.info(msg)
                return ActionResult(extracted_content=msg, include_in_memory=True)
            except AttributeError as e:
                # If browser doesn't have page, try accessing _page
                if hasattr(browser, '_page'):
                    await browser._page.evaluate(f"document.body.style.zoom = '{percentage}%'")
                    msg = f'Successfully zoomed page to {percentage}%'
                    logger.info(msg)
                    return ActionResult(extracted_content=msg, include_in_memory=True)
                else:
                ...
                msg = f'Failed to zoom page to {percentage}%: {str(e)}'
                logger.info(msg)
                return ActionResult(error=msg)
