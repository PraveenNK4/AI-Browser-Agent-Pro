...
def create_agent_settings_tab(webui_manager: WebuiManager):
    ...
    with gr.Group():
        ...
        with gr.Row():
            ...
            use_vision = gr.Checkbox(
                label="Use Vision",
                value=False,  # Disabled by default for local models
    ...
    with gr.Row():
        max_steps = gr.Slider(
            minimum=1,
            maximum=1000,
            value=25,  # Reduced from 100 for efficiency
            step=1,
            label="Max Run Steps",
            info="Maximum number of steps the agent will take (reduce for simple tasks)",
            interactive=True
        )
        max_actions = gr.Slider(
            minimum=1,
            maximum=100,
            value=3,  # Reduced from 10 for efficiency
            step=1,
            label="Max Number of Actions",
            info="Maximum number of actions the agent will take per step (reduce for simple tasks)",
            interactive=True
        )
