...
Generated from: 68cc8cce-8f24-472b-897e-4c44640aefe0.json
...
Usage:
  ...
  python 68cc8cce-8f24-472b-897e-4c44640aefe0.py
"""
...
async def run_automation():
    ...
    # Setup
    run_id = "68cc8cce-8f24-472b-897e-4c44640aefe0"
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if cred:
                ...
                print("Warning: OTCS_USERNAME not set in environment")
            ...
            print(f"Step {step_counter}: input_text")
            time.sleep(1)  # Pause to see action in browser
            await page.wait_for_selector("#otds_password")
            cred = os.getenv("OTCS_PASSWORD")
            if cred:
                await page.fill("#otds_password", cred)
            else:
                print("Warning: OTCS_PASSWORD not set in environment")
            ...
            time.sleep(1)  # Pause to see action in browser
            await page.wait_for_selector("#loginbutton")
            ...
            time.sleep(1)  # Pause to see action in browser
            # Extract structured data from element with validation
            element = await page.query_selector("html.js:nth-child(1) > body:nth-child(2) > table.pageBody:nth-child(6)")
            if element:
                try:
                    extracted_content = await parse_table_data(element)
                    # Validate extraction (check for minimum expected data)
                    if isinstance(extracted_content, str) and len(extracted_content.strip()) > 10:
                        pass  # Valid extraction
                    else:
                        extracted_content = "Warning: Extraction returned minimal data"
                except Exception as e:
                    extracted_content = f"Extraction error: {str(e)}"
            else:
                extracted_content = "ERROR: Target element not found"
        ...
        for attempt_path in save_attempts:
            try:
                ...
                print(f"\nOK: Report generated: {attempt_path}")
                print(f"OK: Report size: {attempt_path.stat().st_size / 1024:.1f} KB")
                saved = True
                break
            except Exception as e:
        ...
        if not saved:
            print("\nWarning: Could not save report to any location")
